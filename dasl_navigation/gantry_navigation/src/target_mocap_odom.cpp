...
int main(int argc, char** argv){
  ros::init(argc, argv, "target_odom");
  ...
  listener.waitForTransform("/mocap_origin", "/target", ros::Time::now(), ros::Duration(3.0));
  ...
  while(n.ok()){
    ...
    listener.lookupTwist("/mocap_origin", "/target", 
    ...
    odom_trans.child_frame_id = "target";
