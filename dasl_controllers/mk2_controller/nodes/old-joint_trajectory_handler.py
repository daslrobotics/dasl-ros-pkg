...
import roslib; roslib.load_manifest('mk2_controller')
import rospy
import copy
import threading
import Queue
...
class MotionControllerSimulator():
    ...
    """
    def __init__(self, num_joints, update_rate = 100, buffer_size = 0):
...
class IndustrialRobotSimulatorNode():
    ...
    """
    def __init__(self):
        rospy.init_node('joint_trajectory_handler')
        ...
        self.motion_ctrl = MotionControllerSimulator(len(self.joint_names), update_rate = motion_update_rate)
	...
	# Subscribe to Joint States
        self.joint_fb_msg = FollowJointTrajectoryFeedback()
        rospy.Subscriber('/joint_states', JointState, self.read_joint_state_data)

        # Publish to mk2 joints
	#self.joint_command_pub = rospy.Publisher('/command', JointState)
    ...
    def read_joint_state_data(self, msg):
        try:
            ...
            #with self.lock:
                ...
                #Joint feedback
            #self.joint_fb_msg.header.stamp = time
            #self.joint_fb_msg.joint_names = msg.name #self.joint_names
            #self.joint_fb_msg.actual.positions = msg.position #self.motion_ctrl.get_joint_positions()
                
                #self.joint_feedback_pub.publish(joint_fb_msg)
                         
        except Exception as e:
            rospy.logerr('Unexpected exception in read joint state data: %s', e)
    ...
    def joint_state_publisher(self):
        ...
        joint_state_msg = JointState()
            ...
            #with self.lock:
                ...
                #self.joint_feedback_pub.publish(self.joint_fb_msg)
...
if __name__ == '__main__':
    try:
        rospy.loginfo('Executing joint_trajectory_handler')
