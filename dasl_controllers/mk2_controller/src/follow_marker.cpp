...
int main(int argc, char **argv)
{
  ...
  enter_coor.pose.pose.position.x=0.38;
...
*/
  ...
  while(nh.ok())
  {
    enter_coor.pose.pose.position.x = xt;
    enter_coor.pose.pose.position.y = yt;
    enter_coor.pose.pose.position.z = zt;

    ROS_INFO("X: %.2f, Y: %.2f, Z: %.2f", xt, yt, zt);
