...
class PID:
	...
	def __init__(self):
		...
		for self.f_len, line in enumerate(self.fh,1):
			...
			self.x_des.append(float(current_line[1]))
		self.fh.close()
	...
	def update_gantry_velocity(self):
		...
		while self.is_running and (self.i < self.f_len):
			...
			self.set_point = self.offset + (self.x_des[self.i+1] - self.x_des[self.i])
			...
			self.gantry_cmd_vel.linear.x = (self.x_des[self.i+1] - self.x_des[self.i]) * 0.03937

			self.gantry_cmd_vel_pub.publish(self.gantry_cmd_vel)
	
			#print self.gantry_cmd_vel.linear.x
