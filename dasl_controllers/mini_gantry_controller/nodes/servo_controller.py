...
class GantryControl():
	def __init__(self):
		...
		if not options.clean and os.path.exists(settingsFile):
			...
			settings['lowestServoID']=1
			settings['highestServoID']=10
		...
		if options.clean or not os.path.exists(settingsFile):
			axes = ['x'] # defines the axis that we are using
			dynamixels_used = []
			for axis in axes:
				number = None
				while not number:
		...
		self.position_dyns = [ settings['x'] ]
		self.orientation_dyns = []
		...
		for actuator in self.myActuators:
			if (actuator.id in self.position_dyns):
			...
			if self.velocity_data:
				for actuator in self.myActuators:
					if actuator.id == self.position_dyns[0]: # x
						actuator.moving_speed = 100 #self.radToDynVel(self.velocity_data.linear.x)
