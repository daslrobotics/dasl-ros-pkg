...
from geometry_msgs.msg import Position
...
class GantryControl():
	def __init__(self):
		...
		if not options.clean and os.path.exists(settingsFile):
			...
			while not baudRate:
				...
				if not brTest:
					...
					baudRate = brTest if 9600 <= brTest <= 1000000 else None
			settings['baudRate'] = baudRate
			settings['highestServoID']=255
		...
		self.net.scan(1,settings['highestServoId'])
		self.myActuators = []

		for dyn in self.net.get_dynamixels():
			print dyn.id
			self.myActuators.append(net[dyn.id])
		if options.clean and not os.pth.exists(settingsFile):
			...
			for axis in axes:
				...
				while not number:
					...
					number = numTest if (0<numTest<=255 and (not numTest in dynamixels_used)) else None
