...
def main(env,options):
    ...
    env.Load('mk2.robot.xml')
    time.sleep(1)

    # Use these commands to capture video
    ...
    with env: # lock environment
        Tgoal = numpy.array([[0,-1,0,-0.25],[-1,0,0,0.25],[0,0,-1,0.5],[0,0,0,1]])
    ...
    # Six-DOF UAV "joints"
    # J0 - gantry_x_axis_joint
    # J1 - gantry_y_axis_joint
    # J2 - gantry_z_axis_joint
    # J3 - gimbal_yaw_joint
    # J4 - gimbal_pitch_joint
    # J5 - gimbal_roll_joint

    # Seven-DOF Manipulator
    # J6 - right_shoulder_pitch_joint
    # J7 - right_shoulder_roll_joint
    # J8 - right_shoulder_yaw_joint
    # J9 - right_elbow_pitch_joint
    # J10 - right_wrist_yaw_joint
    # J11 - right_wrist_pitch_joint
    # J12 - right_wrist_roll_joint

    print 'move UAV to target'
    with env:
        ...
        jointnames = ['gantry_x_axis_joint','gantry_y_axis_joint','gantry_z_axis_joint',
                      'gimbal_yaw_joint','gimbal_pitch_joint','gimbal_roll_joint']
        robot.SetActiveDOFs([robot.GetJoint(name).GetDOFIndex() for name in jointnames])
        basemanip.MoveActiveJoints(goal=[7.0,-1.3,-0.8,0,0,0])
    waitrobot(robot)
